...
const especialidadesMap = especialidadesDetalle.reduce((acc, esp) => {
    ...
    const renderizarTurnos = () => {
        ...
        // 1. Encontrar el OBJETO COMPLETO del médico
        const medico = medicosIniciales.find(m => m.id === turno.medicoId); 
        
        // 2. Obtener los nombres y especialidad
        const medicoNombre = medico ? `${medico.nombres} ${medico.apellidos}` : 'Desconocido';
        const osNombre = osMap[turno.obraSocialId] || 'Particular';
        
        // 3. Obtener el nombre de la especialidad
        let especialidadNombre = 'N/A';
        if (medico && medico.especialidadId) {
            // Usamos el ID del médico para buscar en el mapa de especialidades
            especialidadNombre = especialidadesMap[medico.especialidadId] || 'ID no encontrado';
        }
